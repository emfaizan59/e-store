...
export default class UpdateProduct extends Component {
   ...
   productLoad = () => {
    firebase.database().ref(`products/${this.props.itemNo}`).on('value', (snapshot) => {
        const userObj = snapshot.val();
        checkImg = userObj.Images
       this.setState({pname : userObj.productName , prating : userObj.productRating , returnPolicy : userObj.returnPolicy , productReviews : userObj.productReviews ,price : userObj.Price , sale : userObj.Sale , quantity : userObj.Quantity , catname : userObj.Category , id : userObj.id ,
         load : false  , warranty : userObj.Warranty , content : userObj.Description
    })
 ...
 imgList = []
    ...
    firebase.database().ref(`products/${this.props.itemNo}/Images`).once("value").then((snapshot) => {
        ...
        this.setState({databaseImg : []})
        this.setState({ databaseImg : [...imgList]})
        console.log(imgList)
...
timelimit = null
   delDatabaseImg = (index , url) => {
    ...
    let ind = checkImg.indexOf(url)
    console.log(ind)
    firebase.database().ref(`products/${this.props.itemNo}/Images/${ind}`).remove() 
...
// firebase.database().ref("products/1587752818/Images/"+url).remove()
...
//     {
//         console.log(i)
// firebase.database().ref(`products/1587807366/Images/${i}`).remove() 
//     }
// }
...
if(this.isValid())
{
   ...
   let timeStamp = this.props.itemNo
    

       const promises = [];
         fileArr.forEach(file => {
           const uploadTask = 
            firebase.storage().ref().child(`${this.state.id}/${Math.random()*100}`).put(file);
              ...
              .then(() => {
                console.log("then wala")
                  ...
                  this.timeout = setTimeout(() => {
                    ...
                    console.log("Image List Down")
                    console.log(imgList)

                    
                  

                    imgList.push(...uniqueUrl)
                    
                    
                    var uniqueArray = [];
        
                    // Loop through array values
                    for(var value of imgList){
                    ...
                    console.log("ImgList State Down")
                    console.log(imgList)


                    console.log("Unique State Down")
                    console.log(uniqueArray)

               
                    this.state.dataSet.child(timeStamp).update({
                        id : timeStamp ,
                ...
                // {
                    firebase.database().ref('products/'+timeStamp).child("Images").set(uniqueArray)
                // }
                  

                this.setState({loading : false , successMsg : "Successfully Update Product" , pname : '' , catname : '' , quantity : 0 , price : 0 , 
            warranty : true , returnPolicy : 0 
            })
            fileArr = [] 
            urlArr = []
    ...
    isValid = () => {
        ...
        this.setState({pnameEr : false , catnameEr : false , quantityEr : false , priceEr : false , returnPolicyEr : false , fileEr : false })
        errorMsg = []
        if(pname !== "" && catname !== "" && quantity !== 0 &&  price !==0 && content !== '' &&  returnPolicy !== 0 &&  ( this.state.databaseImg.length >0 || fileArr.length >0) )
        {
            ...
            if(fileArr.length <= 0 || this.state.databaseImg.length <= 0 )
            {
            ...
            if(content == '')
            {
                errorMsg.push("Product Description should not empty.\n")
 ...
 <Container fluid style={{paddingBottom : '30px'}}>
                <Header as="h3">Update Product</Header>
                <Form >
   ...
   <Form.Group widths="equal">
                ...
                <Form.Field label='Category' control='select'   name="catname" value={this.state.catname}    onChange = {this.valueChange}  className={this.state.catnameEr ? "error" : ""}
               >
...
{this.state.databaseImg.length > 0 ? 
<Fragment>
    <h3>Already Uploaded Images</h3>
<Grid columns={2} >

    {this.state.databaseImg.map((element , i) => (

<Grid.Column width={8}>
<div className="imgPreview" style = {{height : '150px' , width : '150px' , marginRight:'10px', background : `url('${element}') no-repeat center/contain`}}>
        <Icon name="delete" color="red" inverted link className="iconDel"  size="large" onClick={() => {this.delDatabaseImg(i , element)}}/>
    </div>
    ...
    />
            ...
            {fileArr.length > 0 && this.state.progress !== 100 ? 
             ...
             {/* <Button type='submit' disabled={this.state.loading}  className={this.state.loading ? 'loading' : ''} color="twitter">Submit</Button> */}
                 <Button type='submit' fluid onClick={this.onUpdateSubmit} disabled={this.state.loading}  className={this.state.loading ? 'loading' : ''}  color="twitter">Update</Button>
